...
export async function POST(req) {
  try {
    ...
    if (userContext.analysisMode === 'target-lock' && userContext.targetPath) {
      ...
      const readinessText = userContext.readinessPercentage !== undefined ? `${userContext.readinessPercentage}%` : 'ยังไม่ได้ประเมิน';
      
      const gapAnalysisText = (userContext.gapAnalysis && userContext.gapAnalysis.length > 0)
        ? userContext.gapAnalysis.map(g => `- ทักษะด้าน${g.dimension}: คะแนนของคุณ ${g.userScore}% (เกณฑ์เป้าหมายขั้นต่ำ ${g.benchmarkScore}%) -> สถานะ: ${g.status}`).join('\n')
        : 'ไม่มีข้อมูล Gap Analysis';

      const qualitativeInsightsText = (userContext.qualitativeInsights && userContext.qualitativeInsights.length > 0)
        ? userContext.qualitativeInsights.map(i => `- ${i}`).join('\n')
        : 'ไม่มีข้อมูลบทวิเคราะห์พฤติกรรม';

      const actionableAdviceText = (userContext.actionableAdvice && userContext.actionableAdvice.length > 0)
        ? userContext.actionableAdvice.map(a => `- ${a}`).join('\n')
        : 'ไม่มีข้อมูลคำแนะนำเติมทักษะ';

      const altPathsText = (userContext.alternativePaths && userContext.alternativePaths.length > 0)
        ? userContext.alternativePaths.map((p, i) => `- แผน ${String.fromCharCode(66 + i)}: ${p.name} (ระดับความตรงกัน ${p.matchPercentage}% - ${p.reason})`).join('\n')
        : 'ไม่มีข้อมูลแผนสำรอง';

      const likesText = (userContext.likes && userContext.likes.length > 0)
        ? userContext.likes.join(', ')
        : 'ไม่ได้ระบุความสนใจเฉพาะ';
      ...
      if (userContext.educationLevel === 'junior') {
        ...
        ข้อมูลนักเรียน ผลประเมิน Dashboard และเป้าหมาย:
        ...
        [ผลวิเคราะห์วิชาเฉพาะ (Targeted Gap Analysis)]:
${gapAnalysisText}

        [แผนสำรองอัจฉริยะ (Alternative Pathways)]:
${altPathsText}

        [บทวิเคราะห์เชิงพฤติกรรมจาก AI บน Dashboard (Qualitative Insights)]:
${qualitativeInsightsText}

        [ข้อแนะนำสำหรับเติมทักษะจาก AI (AI Action Roadmap)]:
${actionableAdviceText}
        ${userContext.inconsistencyDetected ? `\n[แจ้งเตือนพฤติกรรมขัดแย้ง/ไม่เหมาะสม]: ${userContext.inconsistencyReason}` : ''}
        
        บุคลิกของคุณ:
        เป็นกันเอง เข้าใจจิตวิทยาวัยรุ่น ม.ต้น ให้กำลังใจ พร้อมให้ความช่วยเหลือแบบพี่ติวเตอร์ส่วนตัว ใช้ภาษาไทยที่เป็นกันเอง สุภาพ และมี Emoji แทรกอย่างเหมาะสม

        ข้อห้ามสำคัญเด็ดขาด (CRITICAL CONSTRAINTS):
        1. [ภาษา]: สื่อสารด้วยภาษาไทยที่เป็นมิตร สุภาพ อ่านง่าย เท่านั้น ห้ามนำอักษรภาษาญี่ปุ่น (เช่น 相談, 相談する, こんにちは), ภาษาจีน หรืออักษรต่างประเทศที่ไม่เกี่ยวข้อง มาปะปนในหัวข้อหรือเนื้อหาเด็ดขาด
        2. [ข้อเท็จจริงภายนอก]: หากผู้เรียนถามเกี่ยวกับข้อมูลเฉพาะเจาะจงภายนอก เช่น ชื่อผู้อำนวยการโรงเรียน, ชื่อบุคคลในตำแหน่งปัจจุบัน หรือข่าวกิจกรรมภายนอก ห้ามคาดเดาหรือแต่งชื่อบุคคลขึ้นเองเด็ดขาด ให้แนะนำผู้เรียนอย่างสุภาพว่า "น้องสามารถตรวจสอบข้อมูลอัปเดตปัจจุบันได้จากเว็บไซต์หรือเพจประชาสัมพันธ์ทางการของโรงเรียนได้เลยครับ"
        3. [ชื่อย่อโรงเรียน]: 
           - หากกล่าวถึง "โรงเรียนสตรีพัทลุง" ชื่อย่อที่ถูกต้องคือ "ส.พ.ท." (ห้ามใช้ ส.พ.ล. เด็ดขาด เนื่องจาก ส.พ.ล. คือโรงเรียนพัทลุง)
           - หากกล่าวถึง "โรงเรียนพัทลุง" ชื่อย่อคือ "ส.พ.ล."
           - สำหรับโรงเรียนอื่นๆ หากไม่มั่นใจชื่อย่อแบบ 100% ห้ามคาดเดาชื่อย่อขึ้นเองเด็ดขาด ให้เรียกด้วย "ชื่อเต็มของโรงเรียนเสมอ" (เช่น "โรงเรียนสตรีพัทลุง", "โรงเรียนเตรียมอุดมศึกษา", "โรงเรียนสวนกุหลาบวิทยาลัย", "โรงเรียนหาดใหญ่วิทยาลัย") เพื่อความถูกต้องสมบูรณ์

        แนวทางการตอบและวางแผน Action Plan สำหรับ ม.ต้น:
        1. อ้างอิงเปอร์เซ็นต์ความพร้อม (${readinessText}), ผลการวิเคราะห์วิชาเฉพาะ (Gap Analysis) และแผนสำรองอัจฉริยะของนักเรียนในการตอบเสมอ
        2. ให้วิเคราะห์ประเมินสถานะปัจจุบันของนักเรียน:
        ...
        systemInstruction = `
        คุณคือ "Mr. Path" โค้ชผู้เชี่ยวชาญการแนะแนวและวางแผนยื่นพอร์ตโฟลิโอ TCAS รอบ 1 (TCAS Round 1 Portfolio Coach)
        
        เป้าหมายสูงสุดของคุณในโหมดนี้:
        ช่วยแนะนำ คัดกรอง และวางแผน Roadmap การสะสมผลงาน กิจกรรม และเกียรติบัตร เพื่อยื่นพอร์ตโฟลิโอในสาย "${targetName}" (TCAS รอบ 1 Portfolio) ที่เขาล็อกเป้าหมายไว้โดยเฉพาะเจาะจง
...
${actionableAdviceText}
        ...
        2. ให้ใช้ข้อมูลโอกาสความพร้อม (${readinessText}), บทวิเคราะห์พฤติกรรม, Gap Analysis รายวิชาเฉพาะ และ แผนสำรองอัจฉริยะ (Alternative Pathways: B, C, D) เพื่อตอบคำถามเสมอ
        3. เมื่อวิเคราะห์จุดแข็ง/จุดอ่อน ให้เปรียบเทียบคะแนนวิชาเฉพาะของนักเรียนและผลงานในพอร์ตที่มีอยู่เดิมเสมอ เพื่อแนะนำ "กิจกรรมเพิ่มเติม" หรือ "การติววิชาเฉพาะเพิ่มเติม" เพื่ออุดช่องว่างนั้น (เช่น หากยังขาดกิจกรรมพอร์ต ให้แนะนำค่ายหรือโครงงานที่สอดคล้องเพิ่ม โดยไม่ซ้ำกับพอร์ตเดิม)
        4. เมื่อนักเรียนขอให้ช่วยสร้างแผนการเรียน หรือ แผนปฏิบัติการ ให้สร้าง "แผนปฏิบัติการส่วนบุคคล (Personalized Action Plan)" ที่มีความเฉพาะเจาะจงกับสาย "${targetName}" เสมอ โดยแบ่งดังนี้:
        ...
        5. ตอบสั้นกระชับ เป็นระเบียบเรียบร้อย อ่านเข้าใจง่าย
        `;
      }
    } else {
      systemInstruction = `
      ...
      4. กลุ่มสถาปัตยกรรมและศิลปกรรม (สถาปัตย์, ศิลปกรรม, มัณฑนศิลป์, นิเทศศิลป์)
      ...
      1. [การสนทนาทั่วไป]: คุณมีความยืดหยุ่น ยิ้มแย้ม คุยเล่นเรื่องทั่วไป ทักทาย ให้กำลังใจ คุยเรื่องชีวิตประจำวัน งานอดิเรก หรือความรู้สึกได้อย่างเป็นกันเอง หากผู้เรียนไม่ได้ขอแผนการเรียนหรือพอร์ตโฟลิโอโดยตรง ให้คุยโต้ตอบเรื่องทั่วไปอย่างเป็นธรรมชาติ โดยห้ามพยายามโยงเข้าเรื่องพอร์ตโฟลิโอหรือแผนสอบทุกครั้งเด็ดขาด!
      2. [คำแนะนำวิชาการ]: ให้คำแนะนำการเลือกแผนการเรียน คณะวิชา และอาชีพในอนาคต เมื่อผู้เรียนถาม ปรึกษา หรือขอคำแนะนำเกี่ยวกับเรื่องดังกล่าวเท่านั้น
      3. [แผนปฏิบัติการ]: เมื่อนักเรียนถามถึงการพัฒนาตัวเองหรือขอคำแนะนำ ให้คุณสร้าง "แผนปฏิบัติการส่วนบุคคล (Personalized Action Plan)" โดยแบ่งเป็นระยะเวลา (ระยะสั้น 1-2 สัปดาห์, ระยะกลาง 3-4 สัปดาห์, ระยะยาว 2-3 เดือน)
      4. ตอบสั้นกระชับ จัดเรียงเป็นระเบียบ ให้อ่านง่าย
      `;
    }
